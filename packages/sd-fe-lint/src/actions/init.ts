...
const chooseEnableMarkdownlint = async (): Promise<Boolean> => {
  const { enable } = await inquirer.prompt({
    type: 'confirm',
    name: 'enable',
    message: `Step ${++step}. 是否启用 markdownlint (若没有markdown文件则不需要)`,
    default: true,
  });
  return enable;
};
...
export default async function (options: InitOptions) {
  ...
  // 初始化 eslintType
  if (options.eslintType && PROJECT_TYPES.find((item) => item.value === options.eslintType)) {
  ...
  if (!isTest) {
    ...
    log.info(
      `Step ${++step}. Check and handle possible dependency and configuration conflicts in the project `,
    );
